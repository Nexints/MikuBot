...
module.exports = {
	...
	data: new SlashCommandBuilder()
		...
		.addUserOption(option =>
			option
				.setName('user')
				.setRequired(true)
				.setDescription('The person to wave to.'))
	...
	async execute(interaction) {
		...
		// Search for gif
		const results = await searchGif("anime wave");
		let findhello;
		...
		if (!optedOut) {
			...
			description = `${interaction.user.displayName} has waved to ${interaction.options.getUser("user").displayName} ${findhello.value} time(s)! ○( ＾皿＾)っ\n-# Via Klipy API.`;
		}

		// Title?
		let title = `${interaction.user.displayName} waves to ${interaction.options.getUser("user").displayName}! ○( ＾皿＾)っ`;

		// Send Embed!
		const helloEmbed = await embed(title, description, results.url, results.shortURL)
		const embedMessage = await interaction.reply({
			embeds: [helloEmbed], withResponse: true
		});

		// DM Notify
		let notifdesc = `${interaction.user.displayName} waved to you in the channel ${embedMessage.resource.message.url}!`;
