...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('rin')
        .setDescription('Pulls a gif of Rin!')
    ...
    async execute(interaction) {
        ...
        // Search for Gif
        const results = await searchGif("kagamine rin")

        // Configure the gif response
        let description = `Via Klipy API.`;
        let title = `Found a Rin Gif!`;

        // Send gif
        const rin = await embed(title, description, results.url, results.shortURL)
        await interaction.reply({
            embeds: [rin]
        });
        return;
