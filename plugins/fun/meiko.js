...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('meiko')
        .setDescription('Pulls a gif of Meiko!')
    ...
    async execute(interaction) {
        ...
        // Search for Gif
        const results = await searchGif("meiko vocaloid")

        // Configure the gif response
        let description = `Via Klipy API.`;
        let title = `Found a Meiko Gif!`;

        // Send gif
        const meiko = await embed(title, description, results.url, results.shortURL)
        await interaction.reply({
            embeds: [meiko]
        });
        return;
