...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('len')
        .setDescription('Pulls a gif of Len!')
    ...
    async execute(interaction) {
        ...
        // Search for Gif
        const results = await searchGif("kagamine len")

        // Configure the gif response
        let description = `Via Klipy API.`;
        let title = `Found a Len Gif!`;

        // Send gif
        const len = await embed(title, description, results.url, results.shortURL)
        await interaction.reply({
            embeds: [len]
        });
        return;
